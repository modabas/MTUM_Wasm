...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    private async Task GetUser()
    {
        ...
        var result = await SystemAdminService.GetUser(request, default);
    ...
    private async Task Update()
    {
        try
        {
            ...
            if (_form.IsValid)
            {
                var ret = await SystemAdminService.UpdateUserAttributes(_updateUserAttributesRequest, default);
                if (ret.Succeeded)
                {
    ...
    private async Task ToggleEnabled()
    {
        try
        {
            var ret = await SystemAdminService.ChangeUserState(_changeUserStateRequest, default);
            if (ret.Succeeded)
            {
